...
io.sockets.on('connection', function (socket) {
  ...
  // 入室
  //socekt.emit("connect");
  socket.on('enter', function (roomname,name) {
    socket.roomname = roomname;
    socket.join(roomname);
    var msg = name + "が入室しました";
    userHash[socket.id] = name;
    //io.sockets.emit("splash", {value:msg});
    socket.broadcast.emit("splash", {value:msg});
  ...
  socket.on("publish", function(data) {
    console.log("dj;oajdfjafghakfjhj");
    socket.broadcast.emit("splash", {value:data.value});
    io.to(socket.id).emit("self", {value:data.value});
  });
